...
with open('heartdata.txt', 'r') as in_file:
    stripped = (line.strip() for line in in_file)
    lines = (line.split(",") for line in stripped if line)
    with open('heartdata.csv', 'w',newline='') as out_file:
...
def stdev(columnvalues):
    ...
    for i in range(num):
        m=m+pow(float(columnvalues[i])-avg,2)
    variance = m/(float(num-1))
